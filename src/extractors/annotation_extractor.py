...
import fitz  # PyMuPDF (versión actualizada)
import json
import argparse
import sys
...
class PDFAnnotationExtractor:
    ...
    def _extract_annotation_data(self, annot, page_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create a dictionary of coordinates using the rect properties
    ...
    def _extract_highlighted_text(self, annot) -> str:
        ...
        try:
            ...
            # Extract text from the annotation area using the annotation's rectangle
            rect = annot.rect
            text_instances = page.get_text("dict", clip=rect)

            # Extract text from blocks more efficiently
            text_parts = []
...
def main():
    ...
    parser.add_argument(
        "-v", "--version", action="store_true", help="Show PyMuPDF version and exit"
    )

    args = parser.parse_args()
    
    # Show PyMuPDF version if requested
    if args.version:
        print(f"PyMuPDF version: {fitz.version}")  # Accede a la versión de PyMuPDF
        sys.exit(0)
